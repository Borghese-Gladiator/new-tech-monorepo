...
**Scope creep + blast radius (TODO §1g).** The `validate` default mode also parses `brief.md`'s `## Files likely to change` (or `## Scope`) section and compares it against `git diff --name-only <base_ref>...HEAD`. Any unexpected files are appended to `review.md` under a `## Scope creep check` section and a `ScopeCreepChecked` event is recorded. The deeper blast-radius traversal — depth-3 callers of changed symbols — is authored by the reviewer agent itself during `/validate` step 3 via `git diff` + `git grep`, and lives in the same `review.md` under its own `## Blast radius` section.
