...
```bash
# Confirm the tree
find agent-workbench-live/context -name '*.md' | wc -l   # expect 21
...
4. Open `CLAUDE.md` (new file at the repo root). Confirm the three required references: `@context/README.md`, `@context/meta/repo-discovery.md`, `@context/meta/risk-and-approval.md`.
5. `grep -E '^Context:' agent-workbench-live/.claude/commands/*.md` — expect 5 lines (`plan`, `validate`, `start`, `followups`, `new-run`).

If steps 1–5 pass and the unit suite is 198/198 green, the run is delivered.
...
- 2026-05-22 — Build → 21-file `context/` tree, AGENTS+CLAUDE wiring, 5 slash-command `Context:` imports, `tests/test_context_library.py`. Suite 193 → 198 green.
